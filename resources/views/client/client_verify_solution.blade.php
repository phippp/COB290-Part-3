...
@section('content')
    ...
    <div class="page-container"> <!-- this class will center the content i.e align it horizontally and put max width -->
        ...
        <button type="button" class="btn-secondary" id="yes-btn" name = "yes-btn" onclick="location.href = '{{ route('client_problem_view_worked', $problemlog) }}';">Yes</button>
        <button type="button" class="btn-secondary" id="no-btn" name = "no-btn" onclick="location.href = '{{ route('client_problem_view_did_not_work', $problemlog) }}';">No</button>
    </div>
@endsection